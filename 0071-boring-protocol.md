...
In order to convert our settlement process from BOP to DC/HNT, we will be implementing a 1-way burn [BOP-Swap] of the existing BOP token into the dVPN subDAO treasury to be redeemed for $VPN or HNT. This BOP-to-DC/HNT exchange can be handled automatically as dVPN service is rendered, or directly via exchange with the treasury. During Genesis, the $VPN token will be airdropped to early node providers for a set period TBD (~6-12Mo.), after which time the BOP-Swap will no longer be available and the BOP token will become obsolete. Network users will begin paying with DC/HNT, and node providers will be rewarded $VPN, with reward emissions each Epoch in competition with the other subDAOs based on Boring Protocol’s relative utility score, building out the Network of Networks proposed in [HIP51.](https://github.com/helium/HIP/blob/main/0051-helium-dao.md)
