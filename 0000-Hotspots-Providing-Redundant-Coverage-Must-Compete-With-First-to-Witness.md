...
When hotspots have been denied for 'Redundant Coverage', there must be more than *Desired Redundancy* witnesses.  The next fastest hotspot to respond would then be considered for rewards.  If it is not providing *Redundant Coverage*, then it is rewarded as normal.  Otherwise if it is providing *Redundant Coverage*, it must also be the first to respond of all hotspots within *Minimum Distance*. If it is not, the next fastest would be considered, and so on, until either *Desired Redundancy* valid witnesses are obtained or there are no more witnesses to consider.  

There are 16 witnesses to a beacon.  Number 14 is denied as 'Redundant Coverage'.  Number 15 would then be considered, if it passes all is done.
