...
For Helium Mobile unlimited plan we propose to set the limit on rewardable data that each Subscriber can send to the Helium MObile subDAO during the 30-day billing cycle. We propose that this limit is expressed in terms of gigabytes and equal COST_of_Subscription / .5 (price per GB set by MOBILE subDAO). I.e. if I purchased a Helium Mobile unlimited subscription for $40, maximum number of rewardable gigabytes I can send to the Helium network is $40/.5 = 80Gb. This way, the total value of MOBILE rewards received by a hotspot for servicing any unique unlimited plan cannot exceed the price this plan was purchased for, thus elminating the incentive to game. 

The rewardable limit will reset with a start of a new 30-day billing cycle for each Subscriber with an unlimited data plan. The data traffic cap is not limited to a particular Hotspot. It is applicable to all Hotspots that provide data for a specific Subscriber.
