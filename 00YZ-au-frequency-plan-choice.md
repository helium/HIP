...
As has been outlined above, a Yes vote does not preclude future support for AS923, however, such support should be introduced in a manner that does not diminish full AU915 support to allow roaming from all AU915 networks, while introducing support for AS923 using more capable hardware.
...
It is proposed conduct a regional temperature check using the Helium on-chain voting system [Helium Vote](https://heliumvote.com) to allow any wallet to vote on this HIP. See [Voting on Helium](https://docs.helium.com/community-voting/) documentation on Helium Docs.
...
5. Restore faith in Helium: We expect this change would restore confidence in Hotspot owners who purchased Helium-related hardware and have suffered reduced performance since November. We expect a restoration of faith in the Helium governance system and less of a feeling that the Australian users have been ignored.
